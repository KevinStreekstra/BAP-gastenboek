...
<div class="container">
    ...
    <?php
    $dbc   = new PDO('mysql:host=localhost;dbname=22263_database', '22263', 'brsyfrwv') or die ('Error connecting to database');
    $dbc->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
    $stmt = $dbc->prepare("SELECT * FROM 22263_pdo ORDER BY id DESC");
        ...
        if (!empty($naam) && !empty($message)) {
            ...
            foreach($word_array as $word){
                if(in_array(strtolower($word), $list)){
                    $message = preg_replace('/\b'.$word.'\b/i', "bobba", $message);
                }
            }
            //Query samenstellen
            $stmt = $dbc->prepare("INSERT INTO 22263_pdo VALUES (0, :naam, :message)");
